...
import BankAccount ( BankAccount, openAccount, closeAccount
                   , getBalance, incrementBalance, transfer )
...
main = exitProperly $ runTestTT $ TestList
  ...
  , testCase "transfer works" $ do
    ac1 <- openAccount
    ac2 <- openAccount
    checkReturn (Just 100) (incrementBalance ac1 100)
    checkReturn True (transfer ac1 ac2 40)
    checkReturn (Just 40) (getBalance ac2)
    checkReturn (Just 60) (getBalance ac1)
  , testCase "Can't go negative" $ do
    ac <- openAccount
    checkReturn Nothing (incrementBalance ac (negate 10))
  , testCase "Transferring amount you don't have doesn't work" $ do
    ac1 <- openAccount
    ac2 <- openAccount
    checkReturn False (transfer ac1 ac2 1)
    checkReturn (Just 0) (getBalance ac1)
    checkReturn (Just 0) (getBalance ac2)
  ]
